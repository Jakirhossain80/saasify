// FILE: server/services/savedViews.service.ts
import type mongoose from "mongoose";
import { createAuditLog } from "@/server/repositories/auditLogs.repo";
import { createSavedView, deleteSavedView, listSavedViews, togglePinSavedView } from "@/server/repositories/savedViews.repo";
import type { SavedViewDoc } from "@/server/models/SavedView";
...
export async function removeSavedView(input: {
  ...
  return deleteSavedView({ tenantId: input.tenantId, userId: input.userId, viewId: input.viewId });
}
