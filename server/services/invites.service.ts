// FILE: server/services/invites.service.ts
import crypto from "crypto";
import type mongoose from "mongoose";
import { createAuditLog } from "@/server/repositories/auditLogs.repo";
import { createInvite, expireOldInvites, hashToken, listInvitesByTenant, setInviteStatus, findInviteByTokenHash } from "@/server/repositories/invites.repo";
import type { InviteDoc, TenantRole } from "@/server/models/Invite";
import { addMembershipIfNotExists } from "@/server/services/memberships.service";
...
export async function listTenantInvites(input: {
  ...
  return listInvitesByTenant(input.tenantId, { status: input.status, limit: input.limit, offset: input.offset });
}
...
export async function acceptTenantInvite(input: {
  ...
  const updated = await setInviteStatus(input.tenantId, invite._id, "accepted", input.acceptedByUserId);
