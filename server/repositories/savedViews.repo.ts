// FILE: server/repositories/savedViews.repo.ts
import type mongoose from "mongoose";
import { connectToDatabase } from "@/server/db";
import { SavedView, type SavedViewDoc } from "@/server/models/SavedView";
...
export async function deleteSavedView(input: {
  ...
  await connectToDatabase();
  return SavedView.findOneAndDelete({ _id: input.viewId, tenantId: input.tenantId, userId: input.userId }).exec();
}
